...
class testUserClass(object):
    ...
    def testFavoriteCharacters(self):
        ...
        assert self.shal.favorite_characters[self.tohsaka] == self.fsn and self.shal.favorite_characters[
                                                                               self.fujibayashi] == self.clannad_movie
    ...
    def testGender(self):
        assert self.shal.gender is None
        assert self.mona.gender == u"Male"

    def testBirthday(self):
        assert isinstance(self.shal.birthday, datetime.date) and self.shal.birthday == datetime.date(year=1989,
                                                                                                     month=11, day=5)
        assert isinstance(self.mona.birthday, datetime.date) and self.mona.birthday == datetime.date(year=1991, month=8,
                                                                                                     day=11)

    def testLocation(self):
        assert self.shal.location == u'Chicago, IL'
        assert isinstance(self.mona.location, str)

    def testWebsite(self):
        assert len(self.shal.website) > 0
        assert self.shal.website[0]['name'] == u'llanim.us'
        assert self.mona.website is None

    def testJoinDate(self):
        assert isinstance(self.shal.join_date, datetime.date) and self.shal.join_date == datetime.date(year=2008,
                                                                                                       month=5, day=30)
        assert isinstance(self.mona.join_date, datetime.date) and self.mona.join_date == datetime.date(year=2009,
                                                                                                       month=10, day=9)
    ...
    def testLastListUpdates(self):
        ...
        assert self.shal.last_list_updates[self.fate_zero][u'status'] == u'Watching' and \
               self.shal.last_list_updates[self.fate_zero][u'progress'] == 6 and \
               self.shal.last_list_updates[self.fate_zero][u'total'] == 13

        assert isinstance(self.shal.last_list_updates[self.fate_zero][u'time'], datetime.datetime)
        assert self.shal.last_list_updates[self.fate_zero][u'time'] == datetime.datetime(year=2014, month=9, day=5,
                                                                                             hour=14, minute=1, second=0)
        assert self.bebop in self.shal.last_list_updates and self.bebop in self.shal.last_list_updates
        assert self.shal.last_list_updates[self.bebop][u'status'] == u'Completed' and \
               self.shal.last_list_updates[self.bebop][u'progress'] == 26 and self.shal.last_list_updates[self.bebop][
                                                                                  u'total'] == 26
        assert isinstance(self.shal.last_list_updates[self.bebop][u'time'], datetime.datetime) and \
               self.shal.last_list_updates[self.bebop][u'time'] == datetime.datetime(year=2012, month=8, day=20,
                                                                                     hour=11, minute=56, second=0)
    ...
    def testAbout(self):
        if self.shal.about is not None:
            assert isinstance(self.shal.about, str) and len(self.shal.about) > 0
            assert u'retiree' in self.shal.about
        else:
            assert self.shal.about is None
        if self.mona.about is not None:
            assert isinstance(self.mona.about, str) and len(self.mona.about) >= 0
            assert self.mona.about == u'Nothing yet'
        else:
            assert self.mona.about is None

    def testReviews(self):
        assert isinstance(self.shal.reviews, dict) and len(self.shal.reviews) == 0

        assert isinstance(self.smooched.reviews, dict) and len(self.smooched.reviews) >= 0
        if len(self.smooched.reviews) > 0:
            assert self.sao in self.smooched.reviews
            assert isinstance(self.smooched.reviews[self.sao][u'date'], datetime.date) and self.smooched.reviews[self.sao][
                                                                                           u'date'] == datetime.date(
            year=2012, month=7, day=24)
            assert self.smooched.reviews[self.sao][u'people_helped'] >= 0 and self.smooched.reviews[self.sao][
                                                                                u'people_total'] >= 0
            assert self.smooched.reviews[self.sao][u'media_consumed'] == 13 and self.smooched.reviews[self.sao][
                                                                                u'media_total'] == 25
            assert self.smooched.reviews[self.sao][u'rating'] == 6
            assert isinstance(self.smooched.reviews[self.sao][u'text'], str) and len(
                self.smooched.reviews[self.sao][u'text']) > 0
    ...
    #     assert self.kanon in self.shal.recommendations and self.shal.recommendations[self.kanon][
    #                                                            u'anime'] == self.clannad_as
    #     assert isinstance(self.shal.recommendations[self.kanon][u'date'], datetime.date) and \
    #            self.shal.recommendations[self.kanon][u'date'] == datetime.date(year=2009, month=3, day=13)
    #     assert isinstance(self.shal.recommendations[self.kanon][u'text'], str) and len(
    #             self.shal.recommendations[self.kanon][u'text']) > 0
    ...
    #     assert self.ziron in self.shal.friends and isinstance(self.shal.friends[self.ziron][u'last_active'],
    #                                                           datetime.datetime)
    #     assert self.ziron in self.shal.friends and isinstance(self.shal.friends[self.ziron][u'last_active'],
    #                                                           datetime.datetime)
    #     assert self.seraph in self.shal.friends and isinstance(self.shal.friends[self.seraph][u'last_active'],
    #                                                            datetime.datetime) and self.shal.friends[self.seraph][
    #                                                                                       u'since'] == datetime.datetime(
    #         year=2012, month=10, day=13, hour=19, minute=31, second=0)
