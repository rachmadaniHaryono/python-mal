...
class testCharacterClass(object):
    ...
    def testDescription(self):
        assert isinstance(self.spike.description, str) and len(self.spike.description) > 0
        assert isinstance(self.ed.description, str) and len(self.ed.description) > 0
        assert isinstance(self.maria.description, str) and len(self.maria.description) > 0

    def testPicture(self):
        assert isinstance(self.spike.picture, str) and len(self.spike.picture) > 0
        assert isinstance(self.ed.picture, str) and len(self.ed.picture) > 0
        assert isinstance(self.maria.picture, str) and len(self.maria.picture) > 0
